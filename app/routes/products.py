...
def get_db_product(product_id: str):
    db = database.get_db()
    cur = db.execute(
        "SELECT id, name, description, price, in_stock FROM products WHERE id=?",
        (product_id,),
    )
    row = cur.fetchone()
    return row
...
def create_product(product: schemas.ProductCreate, token: str = Depends(auth.oauth2_scheme)):
    ...
    db.execute(
        ...
        (product_id, product.name, product.description, product.price, int(product.in_stock)),
    )
    db.commit()
...
def list_products(token: str = Depends(auth.oauth2_scheme)):
    auth.get_current_user_token(token)
    db = database.get_db()
    cur = db.execute("SELECT id, name, description, price, in_stock FROM products")
    rows = cur.fetchall()
    return [schemas.Product(id=row[0], name=row[1], description=row[2], price=row[3], in_stock=bool(row[4])) for row in rows]
...
def get_product(product_id: str, token: str = Depends(auth.oauth2_scheme)):
    ...
    return schemas.Product(id=row[0], name=row[1], description=row[2], price=row[3], in_stock=bool(row[4]))
...
def update_product(product_id: str, product: schemas.ProductCreate, token: str = Depends(auth.oauth2_scheme)):
    ...
    db.execute(
        ...
        (product.name, product.description, product.price, int(product.in_stock), product_id),
    )
    db.commit()
    return schemas.Product(id=product_id, **product.dict())
